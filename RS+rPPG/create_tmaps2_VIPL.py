...
for filename in filenames:
    ...
    mstmap = np.load(filename)[:,:,0:3]
    if mstmap.shape[1] < 35:
        continue
    ...
    tmap = np.stack((chrom_map,pos_map,lgi_map,pca_map,ica_map,pbv_map),axis=2)


    out_name = filename.replace("mstmap","tmap2").replace(dir,outdir)
    ...
    plt.show()
    exit()"""
    with open("tmap2_completed_vipl.txt", "a") as file_object:
